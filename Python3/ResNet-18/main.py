...
dataset_train = torchvision.datasets.CIFAR10(root='~/.ml_data', train=True, download=True, transform=toTensor)
train_loader = torch.utils.data.DataLoader(dataset_train, batch_size=batch_size, shuffle=True)

dataset_test = torchvision.datasets.CIFAR10(root='~/.ml_data', train=False, download=True, transform=toTensor)
test_loader = torch.utils.data.DataLoader(dataset_test, batch_size=batch_size, shuffle=True)

model = ResNet18(10).cuda()

loss = torch.nn.CrossEntropyLoss()
optimizer = torch.optim.SGD(model.parameters(), lr=0.01, momentum=0.99, weight_decay=0.0001)
