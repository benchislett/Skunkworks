# Skunkworks

# C

## [Triangle Image Evolution](C/Triangle-Image-Evolution/)
![Image Evolution Target](C/Triangle-Image-Evolution/data/target.png)  ![Image Evolution Sample](C/Triangle-Image-Evolution/output/output.png)

## [Langton's Ant](C/Langtons-Ant/)
![Lanton's Ant Alternate Output](C/Langtons-Ant/output/example_4.bmp) ![Lanton's Ant Default Output](C/Langtons-Ant/output/example_5.bmp)

# C++

## [Conway's Game of Life implementation](C++/Game-of-Life/)
![Conway's Game of Life Sample](C++/Game-of-Life/output/output.png)

## [NES Tetris](C++/Tetris/)
![NES Tetris Sample](C++/Tetris/output/output.png)

# Julia

## [Arbitrary Precision Arithmetic on the GPU](Julia/BigIntGPU/)

## [Julia Set Renderer](Julia/Fractal-Render/)
![Julia Set Renderer Sample](Julia/Fractal-Render/output/output.png)

# Python

## [Noise Algorithms](Python3/Noise/)
![Perlin/Fractal Noise Sample](Python3/Noise/output/fractal-256-8.gif)

## [Split-Step Fourier Method NLSE solver](Python3/Split-Operator-Solver/)

## [Video to Ascii converter](Python3/video2ascii/)

## [ResNet-18 Implementation](Python3/ResNet-18/)

## [Image Compression with Convolutional Autoencoders](Python3/Conv-Autoencoder/)

## [UTSC Available Room Finder](Python3/UTSC-Room-Finder/)

